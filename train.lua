...
function Trainer:train(epoch, dataloader)
    ...
    for n, sample in dataloader:run(self.opt) do
        ...
        if sample.input:size(1) == self.opt.batchSize then
            ...
            print(('Best: %.3f | Epoch: [%d][%d/%d]    Time %.3f  Data %.3f  Err %1.4f  top1 %7.3f  top5 %7.3f'):format(100 - bestTop1, epoch, n, trainSize, timer:time().real, dataTime, loss, top1, top5))
...
function Trainer:test(epoch, dataloader)
    ...
    for n, sample in dataloader:run(self.opt) do
        ...
        print(('Best: %.3f | Test: [%d][%d/%d]    Time %.3f  Data %.3f  top1 %7.3f (%7.3f)  top5 %7.3f (%7.3f)'):format(100 - bestTop1, epoch, n, size, timer:time().real, dataTime, top1, top1Sum / N, top5, top5Sum / N))
